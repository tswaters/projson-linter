...
import {showing as showButton, hiding as hideButton} from '../styles/button.less'
...
class ButtonToggle {
  ...
  constructor () {
    ...
    this.closeButtons = Array.from(document.getElementsByClassName('close-button'))
    this.closeButtons.forEach(button => button.addEventListener('click', () => {
  ...
  toggleForm (shouldShow, id) {
    if (!shouldShow) {
      ...
      this.buttons.forEach(button => {
        button.element && document.getElementById(button.element).classList.remove(showForm)
      })      
    } else {
      document.body.classList.add(hideForm)
      const current = this.buttons.find(button => button.id === id)
      current.element && document.getElementById(current.element).classList.add(showForm)      
    }
  }
